...
export class ProductControlSandbox {
  ...
  addItemsToCart(item, param) {
    ...
    for (let i = 0; i < this.selectedProducts.length; i++) {
      if (this.selectedProducts[i].productId === id && this.selectedProducts[i]._totalOptions !== id_totalOptions) {
    ...
    this.selectedProducts = this.selectedProducts.map(_items => {
      if (_items.productId === item.productId && _items._totalOptions === id_totalOptions) {
        exists = true;
    ...
    this.selectedProducts.forEach(_price => {
      if (_price.productId === item.productId && _price._totalOptions === id_totalOptions) {
  ...
  ChangeCount(product, operation) {
    ...
    if (operation) {
      this.selectedProducts = this.selectedProducts.map(_items => {
        if (_items.productId === product.productId && _items._totalOptions === product._totalOptions) {
          _items.productCount += 1;
          this.cartTotal += 1;
        }
        return _items;
